...
def gini(y: np.ndarray) -> float:
    ...
    probas = [cnt / y.shape[0] for cnt in count.values()]
    
    gini_index = 0
    for proba in probas:
        gini_index += proba * (1 - proba)
...
def weighted_impurity(y_left: np.ndarray, y_right: np.ndarray) -> \
    ...
    weighted_impurity = (size_left * left_impurity + size_right * right_impurity) / (size_left + size_right)
...
class MyDecisionTreeNode:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        assert self._depth != None

        if (
            (len(np.unique(y)) == 1) or
            (X.shape[0] < self._meta.min_samples_split) or
            (self._meta.max_depth != None and self._depth >= self._meta.max_depth)
        ):
            self._node_type = NodeType.TERMINAL
            self._predicted_class = mode(y).mode

            class_count = [(y == class_index).sum() for class_index in range(self._meta._n_classes)]
            self._class_proba = np.array([class_count[class_index] / y.shape[0] for class_index in range(self._meta._n_classes)])
            return self
...
class MyDecisionTreeClassifier:
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        if not self._is_trained:
            ...
            answer = []
            for x in X:
                answer.append(self.root.predict_proba(x))
            return np.array(answer)
...
class MyRandomForestClassifier:
    ...
    def predict_proba(self, X: np.ndarray):
        ...
        for i, x in enumerate(X):
            estimator_probas = []
            for estimator in self.estimators:
                estimator_probas.append(estimator.predict_proba(x))
            estimator_mean_probas = np.array(estimator_probas).mean(axis=0)
            probas[i, :] = estimator_mean_probas
